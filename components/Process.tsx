...
const stepsLeft = [
  {
    num: '01',
    icon: <Search className="w-5 h-5 text-slate-800 dark:text-slate-200" />,
    title: 'Découverte',
    desc: 'Discussion sur votre vision, vos objectifs et votre audience pour définir la direction du projet.',
  },
  {
    num: '02',
    icon: <PenTool className="w-5 h-5 text-slate-800 dark:text-slate-200" />,
    title: 'Conception',
...
const stepsRight = [
  {
    num: '03',
    icon: <Code className="w-5 h-5 text-slate-800 dark:text-slate-200" />,
    title: 'Développement',
    desc: 'Code propre, performant et responsive avec des démos régulières pour suivre l\'avancement.',
  },
  {
    num: '04',
    icon: <Rocket className="w-5 h-5 text-slate-800 dark:text-slate-200" />,
    title: 'Livraison',
    ...
    <section className="w-full my-auto flex flex-col px-4 sm:px-8 py-4 relative">
      <div className="max-w-lg mx-auto w-full relative z-10 flex flex-col items-center">
        ...
        >
          ...
          <p className="text-slate-900 dark:text-slate-200 max-w-sm mx-auto font-serif text-xs md:text-sm">
        ...
        <div className="flex flex-col gap-4 w-full">
          {stepsLeft.map((step, idx) => (
            ...
            >
              <div className="text-4xl md:text-5xl font-bold text-slate-200 dark:text-slate-900 group-hover:text-slate-300 dark:group-hover:text-slate-900 transition-colors">
                {step.num}
              </div>
              <div>
                <div className="flex items-center gap-2 mb-1">
                  {step.icon}
                  <h3 className="text-base md:text-lg font-bold text-slate-950 dark:text-white font-serif">
                    {step.title}
                  </h3>
                </div>
                <p className="text-xs md:text-sm text-slate-900 dark:text-slate-200 leading-relaxed font-serif">
                  {step.desc}
                </p>
              </div>
            </motion.div>
